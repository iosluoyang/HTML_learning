...
(function() {
    ...
    function draw(opts) {
        ...
        for (var i = 0; i < num; i++) {
            ...
            // 绘制圆弧
            ctx.arc(0, 0, center, 0,  2 * Math.PI / num, false);
            ...
            // 奖项列表
            //旋转角度
            var roteteangle = 360 / num * i - rotateDeg
            var prize = '<li class="gb-turntable-item" style="transform: rotate('+i*turnNum+'turn)"><span>'+opts.prizes[i].name+'</span></li>'
            html.push(prize);

            // html.push('<li class="gb-turntable-item" style="transform: rotate("'+i*turnNum+'")"> <span>' + opts.prizes[i].name + '</span> </li>');
            if ((i+1) === num) {
    ...
    function events() {
        bind(btn, 'click', function() {
            ...
            fnGetPrize(function(data) {
                ...
                // 计算旋转角度
                deg = deg || 0;
                deg = deg + (360 - deg % 360) + (360 * 10 - data["chooseindex"] * (360 / num))
                runRotate(deg);
            });
